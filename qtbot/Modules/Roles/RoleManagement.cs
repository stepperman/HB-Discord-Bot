...
namespace Discord_Bot
{
    public static class RoleManagement
    {
        ...
        public static Func<CommandArgs, Task> ShowRoles = async e =>
        {
            ...
            if (msgtype == "user")
            {
                await Tools.ReplyAsync(e, "The user category is too big to display it.");
                return;
            }
        ...
        public static Func<CommandArgs, Task> AddRole = async e =>
        {
            ...
            //Parse the role
            IRole roleToAdd = e.Guild.Roles.FirstOrDefault(x => x.Name.ToLower().Contains(string.Join(" ", args).ToLower()));
            list.Add(roleToAdd.Name, new UserRole(roleToAdd.Id, type));
            SaveRoleList(list, type);

            await Tools.ReplyAsync(e, $"Added {roleToAdd.Name} to {type.ToString()}.");
        };
